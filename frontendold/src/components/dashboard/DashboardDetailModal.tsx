...
export default function DashboardDetailModal({ isOpen, type, onClose }: Props) {
  ...
  const printItems = (items: any[]) => {
    ...
    printWindow.document.write(`<!DOCTYPE html><html><head><title>Print - ${getTitle()}</title>
      <style>
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[9000] flex items-center justify-center p-2 sm:p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-6xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
