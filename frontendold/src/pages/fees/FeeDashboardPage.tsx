            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900 mt-1">{data.summary.totalStudents.toLocaleString("en-IN")}</p>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-700 mt-1">{formatCurrency(data.summary.totalReceivable)}</p>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-700 mt-1">{formatCurrency(data.summary.totalCollected)}</p>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-700 mt-1">{formatCurrency(data.summary.outstanding)}</p>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                ...
                {data.monthlyCollection.map((m) => (
                  <div key={m.month} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full flex gap-0.5 items-end h-36">
                      {/* Receivable bar */}
                      <div
                        className="flex-1 bg-purple-200 rounded-t-sm min-h-[4px]"
                        style={{ height: `${(m.receivable / maxMonthlyCollection) * 100}%` }}
                        title={`Receivable: ${formatCurrency(m.receivable)}`}
                      />
                      {/* Collected bar */}
                      <div
                        className="flex-1 bg-green-500 rounded-t-sm min-h-[4px]"
                        style={{ height: `${(m.collected / maxMonthlyCollection) * 100}%` }}
              ...
              <div className="flex items-center gap-4 mt-3 text-xs text-gray-500">
                <span className="flex items-center gap-1"><span className="w-3 h-3 bg-purple-200 rounded-sm"></span> Receivable</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 bg-green-500 rounded-sm"></span> Collected</span>
              </div>
            </div>

            {/* Outstanding By Class */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
              <h3 className="text-base font-semibold text-gray-900 mb-4">Outstanding By Class</h3>
              {/* Donut Circle */}
                ...
                <div className="relative w-28 h-28">
                  <svg className="w-28 h-28 -rotate-90" viewBox="0 0 36 36">
                    ...
                    <path
                      ...
                      stroke="#ef4444"
                      strokeWidth="3"
                      strokeDasharray={`${(data.summary.outstanding / data.summary.totalReceivable) * 100}, 100`}
                    />
                  </svg>
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                      <p className="text-xs text-gray-500">Total</p>
                      <p className="text-sm font-bold text-red-600">{formatCurrency(data.summary.outstanding)}</p>
                    </div>
                  </div>
                </div>
              </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-20 h-2 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${(cls.outstanding / maxClassOutstanding) * 100}%` }}
                        />
                      </div>
                  ...
                  {data.recentCollections.length === 0 ? (
                    ...
                    data.recentCollections.map((item) => (
                      <tr key={item.receiptNo} className="hover:bg-gray-50">
                        <td className="px-4 py-3 text-sm font-mono text-blue-600">{item.receiptNo}</td>
