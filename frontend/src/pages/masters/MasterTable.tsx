  ...
  return <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xs border border-gray-200 dark:border-slate-700 overflow-hidden w-full flex flex-col">
    {isTimetableSlot ? <>
      <div className="md:hidden divide-y divide-gray-200 dark:divide-slate-700">
        {entries.map((entry, index) => <div key={entry.id || index} className="p-4 space-y-3">
          <div className="flex items-center justify-between gap-3"><span className="text-xs font-semibold text-gray-400">#{(pagination.page - 1) * pagination.limit + index + 1}</span><div className="flex items-center gap-2"><button onClick={() => onToggle(entry.id)} className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${entry.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>{entry.isActive ? <ToggleRight size={12} /> : <ToggleLeft size={12} />}{entry.isActive ? "Active" : "Inactive"}</button>{renderActions(entry)}</div></div>
          <div className="grid grid-cols-2 gap-x-4 gap-y-3">{visibleFields.map(field => <div key={field.name} className="min-w-0"><div className="text-[10px] uppercase tracking-wide text-gray-400 dark:text-gray-500">{field.label}</div><div className="text-sm font-medium text-gray-700 dark:text-gray-200 truncate">{formatValue(entry[field.name], field, entry)}</div></div>)}</div>
