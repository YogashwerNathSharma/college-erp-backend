...
const AddEditTeacher = () => {
  ...
  // Fetch classes/subjects strictly for the year selected in this form.
  const fetchOptions = async (yearId?: string) => {
    ...
    try {
      ...
      const [subRes, classRes] = await Promise.all([
        axios.get(getFullUrl("/api/subjects"), { ...authHeaders, params }),
        axios.get(getFullUrl("/api/class"), { ...authHeaders, params }),
      ]);

      const subs = subRes.data?.data?.data || subRes.data?.data || subRes.data || [];
      const cls = classRes.data?.data?.data || classRes.data?.data || classRes.data || [];
  ...
  // Use the global academic-year list when available.
  useEffect(() => {
    if (globalAcademicYears.length > 0) {
      setAcademicYears(globalAcademicYears);
      if (!isEdit && !academicYearId && selectedAcademicYearId) {
        setAcademicYearId(selectedAcademicYearId);
      }
    }
  }, [globalAcademicYears, selectedAcademicYearId, isEdit, academicYearId]);

  // Fallback: the form must remain usable even if the global context has not
  // loaded its years yet (or the provider was mounted before authentication).
  useEffect(() => {
    ...
    const loadAcademicYears = async () => {
      try {
        ...
        if (!isEdit && fallback && !academicYearId) {
          setAcademicYearId(fallback.id);
        }
      } catch (err) {
  ...
  // Changing the Academic Year immediately reloads classes and subjects.
  useEffect(() => {
    setSelectedClasses([]);
    setSelectedSubjects([]);
  ...
  const fetchTeacher = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (t.photoUrl) {
          const fullUrl = t.photoUrl.startsWith("http") ? t.photoUrl : `${t.photoUrl}`;
  ...
  const toggleSubject = (subId: string) => {
    setSelectedSubjects((prev) => prev.includes(subId) ? prev.filter((s) => s !== subId) : [...prev, subId]);
  };

  const toggleClass = (clsId: string) => {
    setSelectedClasses((prev) => prev.includes(clsId) ? prev.filter((c) => c !== clsId) : [...prev, clsId]);
  };
  ...
  if (fetching) {
    return <div className="flex justify-center items-center py-20"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-600"></div></div>;
  }

  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        <button onClick={() => navigate("/teachers")} className="p-2 hover:bg-gray-100 rounded-lg transition"><FiArrowLeft size={20} /></button>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow p-6 space-y-6">
            ...
            <div className="md:col-span-3 flex justify-end">
              <div className="flex flex-col items-center">
                <div className="w-28 h-28 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden bg-gray-50">
                  {photoPreview ? <img src={photoPreview} alt="Preview" className="w-full h-full object-cover" /> : <div className="text-center text-gray-400"><FiUpload size={24} className="mx-auto mb-1" /><p className="text-xs">Choose Photo</p></div>}
                </div>
        ...
        <div>
          ...
          <div className="mb-4">
            ...
            <select value={academicYearId} onChange={(e) => setAcademicYearId(e.target.value)} className="w-full max-w-sm px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none">
              <option value="">{academicYears.length === 0 ? "Loading Academic Years..." : "Select Academic Year"}</option>
              {academicYears.map((year) => <option key={year.id} value={year.id}>{year.name}</option>)}
            </select>
            {academicYears.length === 0 && <p className="text-xs text-gray-400 mt-1">Academic years are loading. Please wait a moment.</p>}
          </div>

          <div className="mb-4">
            <div className="flex justify-between items-center mb-2">
              <label className="block text-sm font-medium text-gray-700">Assign Classes <span className="text-red-500">*</span><span className="text-xs text-gray-400 ml-1">(Select classes first, then subjects will appear)</span></label>
              {classes.length > 0 && <button type="button" onClick={selectAllClasses} className="text-xs text-green-600 hover:underline">{selectedClasses.length === classes.length ? "Deselect All" : "Select All"}</button>}
            </div>
            <div className="border border-gray-300 rounded-lg p-3 max-h-48 overflow-y-auto">
              {classes.length === 0 ? <p className="text-sm text-gray-400">{academicYearId ? "No classes available for this academic year" : "Select an academic year first"}</p> : <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">{classes.map((cls) => <label key={cls.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer border transition ${selectedClasses.includes(cls.id) ? "bg-green-50 border-green-300" : "bg-white border-gray-200 hover:border-gray-300"}`}><input type="checkbox" checked={selectedClasses.includes(cls.id)} onChange={() => toggleClass(cls.id)} className="rounded text-green-600" /><span className="text-sm">{cls.name}</span></label>)}</div>}
            </div>
            {selectedClasses.length > 0 && <p className="text-xs text-gray-500 mt-1">{selectedClasses.length} class(es) assigned</p>}
          </div>

          <div>
            <div className="flex justify-between items-center mb-2">
              <label className="block text-sm font-medium text-gray-700">Subjects<span className="text-xs text-gray-400 ml-1">(Shows subjects for selected classes)</span></label>
              {filteredSubjects.length > 0 && <button type="button" onClick={selectAllSubjects} className="text-xs text-primary-600 hover:underline">{selectedSubjects.length === filteredSubjects.length ? "Deselect All" : "Select All"}</button>}
            </div>
            <div className="border border-gray-300 rounded-lg p-3 max-h-48 overflow-y-auto">
        ...
        <div className="flex gap-3 pt-4 border-t">
          <button type="submit" disabled={loading} className="flex items-center gap-2 bg-primary-600 text-white px-6 py-2.5 rounded-lg hover:bg-primary-700 transition disabled:opacity-50"><FiSave size={18} />{loading ? "Saving..." : isEdit ? "Update Teacher" : "Save Teacher"}</button>
          <button type="button" onClick={() => navigate("/teachers")} className="px-6 py-2.5 border border-gray-300 rounded-lg hover:bg-gray-50 transition">Cancel</button>
        </div>
      </form>
    </div>
  );
