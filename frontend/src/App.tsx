...
const ExamSchedulePrint = lazyWithRetry(() => import("./pages/exams/ExamSchedulePrint"));
...
function ProtectedRoute() {
  ...
  const userRole = userData ? JSON.parse(userData)?.role : null;
  const adminRoutes = [
    "/dashboard", "/tenants", "/students", "/teachers", "/fees", "/exams", "/settings", "/classes",
    "/subjects", "/academic-years", "/attendance", "/reports", "/library", "/transport", "/backup",
    "/subscriptions", "/hostel", "/hr", "/communication", "/certificates", "/inventory", "/admission",
    "/gate-pass", "/events", "/helpdesk", "/workflow", "/form-builder", "/report-builder", "/audit",
    "/scheduler", "/dashboard-builder", "/qr-barcode", "/payment-gateway", "/file-manager", "/notifications",
    "/import-export", "/queue-monitor", "/digital-signature", "/languages", "/ai-assistant", "/masters"
  ];
  ...
  return (
    <div className="flex min-h-screen min-h-[100dvh] relative bg-gray-50 dark:bg-slate-900 transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col min-w-0 main-content-wrapper md:ml-[260px] transition-all duration-300 bg-slate-50 dark:bg-slate-950 min-h-screen">
        ...
        <button
          className={`hamburger-btn fixed top-3 left-3 z-[9999] p-2.5 bg-primary-600 text-white rounded-xl shadow-lg md:hidden tap-target active:scale-95 transition-all duration-300 ${
        ...
        <Routes>
          ...
          <Route element={<ProtectedRoute />}>
            ...
            <Route element={<Layout />}>
              ...
              <Route path="/exam-schedule-print" element={<ExamSchedulePrint />} />
