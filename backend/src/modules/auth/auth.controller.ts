...
export const registerTenant = async (req: Request, res: Response) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      try { if (logoFile) logoUrl = await uploadToCloudinary(logoFile.buffer, "tenants"); }
      catch (e: any) { console.warn("Logo upload failed (Cloudinary):", e.message); }
      try { if (bgFile) bgUrl = await uploadToCloudinary(bgFile.buffer, "tenants"); }
      catch (e: any) { console.warn("Background upload failed (Cloudinary):", e.message); }
