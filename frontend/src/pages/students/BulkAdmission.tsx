...
export default function BulkAdmission() {
  ...
  const parsePreview = async (selected: File) => {
    ...
    const preferred = workbook.Sheets["Student_List"] || workbook.Sheets["Students"] || workbook.Sheets[workbook.SheetNames[0]];
  ...
  const handleFile = async (selected: File) => {
    if (!/\.(xlsx|csv)$/i.test(selected.name)) return toast.error("Please select a .xlsx or .csv file. Legacy .xls is not supported.");
    if (selected.size > 10 * 1024 * 1024) return toast.error("Maximum file size is 10 MB");
    try {
      await parsePreview(selected);
      setFile(selected);
      setResult(null);
      toast.success("Student file loaded. Preview is ready.");
    } catch (e: any) {
      setFile(null);
  ...
  const startImport = async () => {
    if (!file) return toast.error("Select your student Excel/CSV file first");
    ...
    try {
      ...
      const res = await axios.post(getFullUrl("/api/import-export/real-student-import"), form, auth());
      ...
      toast.error(e.response?.data?.message || "Real student import failed");
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <PageHeader title="Real Student Excel Import" subtitle="Import RMS student data; Class/Section are read automatically from the Excel file" icon={<Upload className="w-5 h-5" />} breadcrumbs={[{ label: "Students", path: "/students" }, { label: "Bulk Admission" }]} actions={<button onClick={downloadTemplate} className="flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700"><Download className="w-4 h-4" />Download ERP Template</button>} />
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-900"><b>Safe import:</b> existing students are matched by Admission Number when present, or Aadhaar when Admission Number is absent. Matching records are updated and their current academic-year enrollment is moved to the Excel Class/Section. No existing student is deleted.</div>
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 space-y-6">
        ...
        <div className="border-2 border-dashed rounded-2xl p-10 text-center cursor-pointer hover:border-indigo-400" onClick={() => inputRef.current?.click()}><FileSpreadsheet className="w-12 h-12 mx-auto text-indigo-600 mb-3" /><p className="font-semibold">{file ? file.name : "Choose your student Excel/CSV file"}</p><p className="text-sm text-slate-500 mt-1">Supports .xlsx or .csv — maximum 10 MB. For legacy .xls, save it as .xlsx first.</p><input ref={inputRef} type="file" accept=".xlsx,.csv" className="hidden" onChange={(e) => e.target.files?.[0] && handleFile(e.target.files[0])} /></div>
