...
const SubjectsPage: React.FC = () => {
  ...
  const fetchSubjects = useCallback(async () => {
    try {
      ...
      const data = res.data;
      console.log("SUBJECTS API response:", { success: data.success, count: Array.isArray(data.data) ? data.data.length : "not array", sample: data.data?.[0] });
      if (data.success) {
  ...
  const filteredSubjects = subjects.filter((subject) => {
    const matchesYear = filterYearId === "all" || subject.academicYearId === filterYearId || !subject.academicYearId;
    const matchesClass =
  ...
  const handleBulkYearChange = async (yearId: string) => {
    setBulkAcademicYearId(yearId);
    setBulkSelectedClassIds([]);
    if (yearId) {
      const cls = await fetchClassesForYear(yearId);
  ...
  const toggleAllBulkClasses = () => {
    if (bulkSelectedClassIds.length === bulkClasses.length) {
      setBulkSelectedClassIds([]);
    } else {
      setBulkSelectedClassIds(bulkClasses.map((c) => c.id));
    }
  };

  const handleBulkSubmit = async () => {
    const names = bulkSubjectNames.split("\n").map((s) => s.trim()).filter(Boolean);
    if (names.length === 0) { showToast("error", "Enter at least one subject name"); return; }
    if (bulkSelectedClassIds.length === 0) { showToast("error", "Select at least one class"); return; }
    if (!bulkAcademicYearId) { showToast("error", "Select academic year"); return; }

    setBulkSubmitting(true);
    try {
      const res = await axios.post(`${API_BASE}/api/subjects/bulk`, {
        names,
        classIds: bulkSelectedClassIds,
        academicYearId: bulkAcademicYearId,
      }, getAuthHeaders());
      const data = res.data?.data;
      showToast("success", `${data?.created || 0} subjects created, ${data?.skipped || 0} duplicates skipped`);
      await fetchSubjects();
      closeBulkModal();
    } catch (err: any) {
      showToast("error", err?.response?.data?.message || "Failed to bulk create subjects");
    } finally {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-xl hover:bg-gray-50 transition-all"
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <div>
                <h3 className="text-lg font-semibold text-gray-900">Bulk Add Subjects</h3>
                <p className="text-xs text-gray-500 mt-0.5">Add multiple subjects to multiple classes at once</p>
              </div>
              <button onClick={closeBulkModal} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleBulkYearChange(e.target.value)}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Subject Names <span className="text-xs text-gray-400">(one per line)</span>
                </label>
                <textarea
                  value={bulkSubjectNames}
                  onChange={(e) => setBulkSubjectNames(e.target.value)}
                  rows={5}
                  placeholder={"Hindi\nEnglish\nMathematics\nScience\nSocial Science\nComputer"}
                  className="w-full rounded-lg border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm font-mono"
                />
                <p className="text-xs text-gray-400 mt-1">
                  {bulkSubjectNames.split("\n").filter((s) => s.trim()).length} subjects entered
                </p>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    type="button"
                    onClick={toggleAllBulkClasses}
                    className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-primary-50 text-primary-700 hover:bg-primary-100 transition-colors"
                  >
                    {bulkSelectedClassIds.length === bulkClasses.length && bulkClasses.length > 0 ? (
                ...
                <span className="text-xs text-gray-500 mb-2 block">
                  {bulkSelectedClassIds.length} of {bulkClasses.length} selected
                </span>
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                  {bulkClasses.map((cls) => {
                    const isSelected = bulkSelectedClassIds.includes(cls.id);
                    return (
                      <button
                        key={cls.id}
                        type="button"
                ...
                {bulkClasses.length === 0 && bulkAcademicYearId && (
                  <p className="text-xs text-amber-600 mt-2">No classes found for selected year.</p>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t bg-gray-50 rounded-b-2xl">
              <button onClick={closeBulkModal} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
                Cancel
              </button>
              <button
                onClick={handleBulkSubmit}
                disabled={bulkSubmitting}
                className="inline-flex items-center gap-2 px-5 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {bulkSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Layers className="w-4 h-4" />}
                {bulkSubmitting ? "Creating..." : `Add to ${bulkSelectedClassIds.length} Classes`}
              </button>
            </div>
          </div>
        </div>
