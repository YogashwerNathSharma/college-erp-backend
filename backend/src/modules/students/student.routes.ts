...
router.post("/:id/photo", (req: any, res: any) => {
  ...
  uploadPhoto(req, res, async (err: any) => {
    ...
    try {
      ...
      console.log("[Photo Route] ✅ Upload success:", result);

      res.json({ success: true, data: result });

    } catch (error: any) {

      console.error("[Photo Route] ❌ Upload error:", error.message);
...
router.post("/", allowRoles("ADMIN", "SUPER_ADMIN"), checkLimit("students"), validate(createStudentSchema), createStudentHandler);
...
router.put("/:id", allowRoles("ADMIN", "SUPER_ADMIN"), validate(updateStudentSchema), updateStudentHandler);
