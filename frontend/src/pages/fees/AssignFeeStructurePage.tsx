...
const AssignFeeStructurePage: React.FC = () => {
  ...
  const handleAssignAll = async () => {
    ...
    // Get all pending (unassigned) students' enrollment IDs
    const pendingStudentIds = students
      .filter((s) => s.assignmentStatus === "NOT_ASSIGNED")
      .map((s) => s.id);

    if (pendingStudentIds.length === 0) {
      toast.error("No pending students to assign");
      return;
    }

    setAssigning(true);
    try {
      const payload: any = { enrollmentIds: pendingStudentIds };
      if (!allFeeHeadsSelected()) {
        payload.selectedItems = selectedItems;
      }
      const res = await axios.post(`${API}/fees/assign/students`, payload);
      if (res.data.success) {
        toast.success(res.data.data.message || "Fees assigned to class");
      }
      fetchStudents();
    } catch (error: any) {
  ...
  // ═══ DELETE: Remove assigned fees for selected students ═══
  const handleDeleteSelected = async () => {
    const assignedSelected = selectedStudents.filter((id) => {
      const s = students.find((st) => st.id === id);
      return s?.assignmentStatus === "ASSIGNED";
    });

    if (assignedSelected.length === 0) {
      toast.error("Please select assigned students to delete fees");
      return;
    }

    if (!window.confirm(`Are you sure you want to DELETE assigned fees for ${assignedSelected.length} student(s)? Only unpaid fees will be removed.`)) {
      return;
    }

    setDeleting(true);
    try {
      const res = await axios.delete(`${API}/fees/assign/students`, { data: { enrollmentIds: assignedSelected } });
      if (res.data.success) {
        toast.success(res.data.data.message || "Fees deleted successfully");
        setSelectedStudents([]);
        fetchStudents();
      }
    } catch (error: any) {
      toast.error(error.response?.data?.error || "Failed to delete fees");
    } finally { setDeleting(false); }
  };

  // DELETE single student's fees
  const handleDeleteSingle = async (enrollmentId: string, studentName: string) => {
    if (!window.confirm(`Delete all unpaid fees for ${studentName}?`)) return;

    setDeleting(true);
    try {
      const res = await axios.delete(`${API}/fees/assign/students`, { data: { enrollmentIds: [enrollmentId] } });
      if (res.data.success) {
        toast.success(res.data.data.message || "Fees deleted");
        fetchStudents();
      }
    } catch (error: any) {
            ...
            {loadingStudents ? (
              ...
              <div>
                          ...
                          <td className="px-3 py-2 text-center">
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              {student.assignmentStatus === "ASSIGNED" && (
                                <button onClick={() => handleDeleteSingle(student.id, student.studentName)} disabled={deleting} className="p-0.5 text-red-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors disabled:opacity-50" title="Delete assigned fees">
                ...
                <div className="border-t bg-gray-50 px-4 py-3 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button onClick={handleDeleteSelected} disabled={deleting || selectedStudents.length === 0} className="px-3 py-1.5 text-xs font-medium bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50">
                      {deleting ? "Deleting..." : `Delete Selected (${selectedStudents.filter((id) => students.find((s) => s.id === id)?.assignmentStatus === "ASSIGNED").length})`}
                    </button>
