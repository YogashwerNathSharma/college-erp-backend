...
// DELETE /api/fees/assign/students — Remove assigned fees (only unpaid ones)
export const unassignFeesController = async (req: any, res: any) => {
  try {
    const tenantId = req.user?.tenantId;
    ...
    const result = await unassignFeesForStudents(enrollmentIds, tenantId);
    res.json({ success: true, data: result });
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
