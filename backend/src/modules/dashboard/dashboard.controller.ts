...
export const getDashboard = async (
  ...
  try {
    ...
    // ═══ REQUEST TIMEOUT: Return fallback if DB is slow ═══
    const TIMEOUT_MS = 20000;
    let timeoutTriggered = false;
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => { timeoutTriggered = true; reject(new Error("DASHBOARD_TIMEOUT")); }, TIMEOUT_MS);
    });
    ...
    const dashData = await Promise.race([
      cached(`dashboard:${tenantId}`, 30000, async () => {
    ...
    if (err.message === "DASHBOARD_TIMEOUT") {
      console.error("⏱️ Dashboard TIMEOUT after 20s — returning empty data");
      return res.json({
        success: true,
        data: {
          ...
          genderData: [], classWiseStrength: [], attendanceTrend: [],
          totalTeachers: 0, attendanceToday: null, notifications: [],
          todayTimetable: [], upcomingEvents: [], upcomingExams: [],
          todayBirthdays: [], tenant: null,
          _timeout: true, _message: "Dashboard data is loading slowly. Please refresh in 30 seconds.",
        },
      });
    }
