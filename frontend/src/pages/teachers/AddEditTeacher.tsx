...
const AddEditTeacher = () => {
  ...
  useEffect(() => {
    if (globalAcademicYears.length > 0) { setAcademicYears(globalAcademicYears); if (!isEdit && !academicYearId && selectedAcademicYearId) setAcademicYearId(selectedAcademicYearId); }
  }, [globalAcademicYears, selectedAcademicYearId, isEdit, academicYearId]);

  useEffect(() => {
    if (globalAcademicYears.length > 0) return;
    let cancelled = false;
    const load = async () => { try { const res = await axios.get(getFullUrl("/api/academic"), authHeaders); const data = res.data?.data || res.data || []; if (!Array.isArray(data) || cancelled) return; setAcademicYears(data); const fallback = data.find((y: AcademicYearOption) => y.isCurrent) || data.find((y: AcademicYearOption) => y.isActive) || data[0]; if (!isEdit && fallback && !academicYearId) setAcademicYearId(fallback.id); } catch (err) { console.error("Failed to fetch academic years:", err); } };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = isEdit ? await axios.put(getFullUrl(`/api/teacher/${id}`), formData, config) : await axios.post(getFullUrl("/api/teacher"), formData, config);
