...
const RECENT_COLORS = [
  ...
  { iconBg: "bg-fuchsia-500", lightBg: "bg-fuchsia-950/50" },
  { iconBg: "bg-lime-500", lightBg: "bg-lime-50 dark:bg-lime-950/50" },
  { iconBg: "bg-pink-500", lightBg: "bg-pink-950/50" },
];
...
// Keep the existing generic master engine intact. Timetable Slot is explicitly
// normalized here because its relation IDs must always be selectable records.
function getEffectiveFields(modelKey: string, configuredFields: FieldConfig[]): FieldConfig[] {
  ...
  if (modelKey === "timetable-slot-master") {
    const timetableFields: FieldConfig[] = [
      {
        name: "dayOfWeek", label: "Day", type: "select", required: true,
        options: [
      ...
      {currentView === "categories" && (
        <div className="max-w-7xl mx-auto space-y-6 animate-fadeIn">
          <div className="border-b border-slate-200 dark:border-slate-800 pb-4 flex items-center gap-3">
            <button onClick={() => navigate("/dashboard")} className="md:hidden p-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-600 dark:text-slate-300 transition-colors flex-shrink-0 tap-target" aria-label="Back"><ArrowLeft size={18} /></button>
            <div>
              <h1 className="text-lg md:text-2xl font-bold flex items-center gap-2"><Grid className="text-indigo-500" size={24} />Master Control Setup</h1>
              <p className="text-xs text-slate-400 mt-1">Select any architecture base matrix block to handle child structural models</p>
            </div>
          </div>
          <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-5 gap-1.5 sm:gap-3">
            {categories.map((category, index) => {
              ...
              return <button key={category.id} onClick={() => handleCategoryClick(category)} className={`flex flex-col items-center gap-1 py-2 sm:py-3 px-1 sm:px-2 rounded-lg ${color.lightBg} hover:scale-105 transition-all duration-200 group relative outline-none active:scale-95 cursor-pointer`}><div className={`w-7 h-7 sm:w-9 sm:h-9 md:w-10 md:h-10 rounded-md sm:rounded-lg ${color.iconBg} flex items-center justify-center`}>{getCategoryIcon(category.icon, 16)}</div><span className="text-[9px] sm:text-[10px] md:text-xs font-medium text-slate-600 dark:text-slate-300 truncate w-full text-center leading-tight">{category.label}</span></button>;
            })}
          </div>
        </div>
      )}
      ...
      {currentView === "child_grid" && selectedCategory && (
        <div className="max-w-7xl mx-auto space-y-6 animate-fadeIn">
          <div className="flex items-center gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
            <button onClick={handleBackToCategories} className="p-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors cursor-pointer flex items-center justify-center" title="Back to Main Menu"><ArrowLeft size={20} /></button>
            <div><div className="flex items-center gap-2"><span>{getCategoryIcon(selectedCategory.icon, 20, "text-indigo-400")}</span><h1 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white">{selectedCategory.label}</h1></div><p className="text-xs text-slate-400 mt-0.5">Select a master target collection mapping below</p></div>
          </div>
          <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-5 gap-1.5 sm:gap-3">
      ...
      {currentView === "table_view" && selectedModel && (
        <div className="max-w-7xl mx-auto animate-fadeIn">
          <div className="flex items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-3"><button onClick={handleBackToChildGrid} className="p-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-500 dark:text-slate-400"><ArrowLeft size={20} /></button><div><h1 className="text-lg md:text-xl font-bold">{selectedModelLabel}</h1><p className="text-xs text-slate-400">Master data management</p></div></div>
            <div className="flex items-center gap-2"><button onClick={handleCreate} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700"><Plus size={18} />Add</button><button onClick={handleExport} className="hidden sm:flex items-center gap-2 px-4 py-2 border rounded-xl"><Download size={18} />Export</button></div>
          </div>
          <MasterTable entries={entries} fields={fields} loading={loading} search={search} onSearch={setSearch} showInactive={showInactive} onShowInactive={setShowInactive} pagination={pagination} onPageChange={(page) => fetchEntries(selectedModel, page)} onEdit={handleEdit} onDelete={handleDelete} onToggle={handleToggle} onClone={handleClone} />
        </div>
      )}

      {showForm && selectedModel && <MasterForm fields={fields} initialData={editingEntry} onSubmit={handleFormSubmit} onClose={() => { setShowForm(false); setEditingEntry(null); }} loading={formLoading} title={`${editingEntry ? "Edit" : "Add"} ${selectedModelLabel}`} />}
    </div>
  );
}
